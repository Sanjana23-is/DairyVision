...
export default function ObservationListPage() {
  ...
  // Reset pagination when search/filter/sort parameters change
  useEffect(() => {
    setCurrentPage(1);
  }, [search, selectedCow, selectedHealth, sortOrder]);

  const totalPages = Math.max(1, Math.ceil(filteredObservations.length / pageSize));

  const visibleObservations = useMemo(() => {
    if (!isExpanded) {
      return filteredObservations.slice(0, 5);
    }
    const start = (currentPage - 1) * pageSize;
    return filteredObservations.slice(start, start + pageSize);
  }, [filteredObservations, isExpanded, currentPage, pageSize]);
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200/90 dark:border-[#27272A] bg-white dark:bg-[#151719] shadow-xs">
              {/* Table Subheader Bar */}
              <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-slate-100 dark:border-[#27272A] bg-slate-50/60 dark:bg-[#1B1D20]/50">
                <div className="flex items-center gap-2.5">
              ...
              {/* Table Footer: View All Toggle / Pagination Controls */}
