...
function FinancialImpactCard({ impact, isHerd = false, cowsCount = 0 }: { impact: FinancialImpact; isHerd?: boolean; cowsCount?: number }) {
  ...
  return (
    <div className="rounded-3xl border border-emerald-100 dark:border-[#27272A] bg-gradient-to-br from-emerald-50/60 via-white to-slate-50 dark:from-[#151719] dark:via-[#1B1D20] dark:to-[#151719] p-6 shadow-sm space-y-4">
      {/* Header with Classification Badge */}
      <div className="flex flex-wrap items-center justify-between gap-2 border-b border-emerald-100 dark:border-[#27272A] pb-3">
          ...
          <span
            className={`rounded-full px-3 py-1 text-xs font-bold border ${
              isPositive
                ...
                : isNegative
                ? "bg-rose-100 dark:bg-rose-950/60 text-rose-800 dark:text-rose-300 border-rose-200 dark:border-rose-800/40"
    ...
    <DashboardLayout>
      <div className="space-y-8">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
          ...
          {/* Mode Switcher Tabs */}
          <div className="flex items-center rounded-2xl bg-slate-100 dark:bg-[#1B1D20] p-1.5 border border-slate-200 dark:border-[#27272A]">
            <button
              onClick={() => setMode("cow")}
              className={`flex items-center gap-2 rounded-xl px-4 py-2 text-xs font-bold transition ${
                mode === "cow"
                  ? "bg-white dark:bg-[#151719] text-emerald-800 dark:text-emerald-300 shadow-sm border border-slate-200/60 dark:border-[#27272A]"
        ...
        {(mode === "cow" ? cowMutation.error : herdMutation.error) && (
          <div className="flex items-center gap-3 rounded-3xl border border-rose-200 dark:border-rose-900/50 bg-rose-50 dark:bg-rose-950/30 p-4 text-rose-950 dark:text-rose-200 shadow-sm">
        ...
        {extrapWarning && (
          <div className="flex items-center gap-3 rounded-3xl border border-amber-200 dark:border-amber-900/50 bg-amber-50 dark:bg-amber-950/30 p-4 text-amber-950 dark:text-amber-200 shadow-sm">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 rounded-3xl border border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#151719] p-4 shadow-sm">
          <span className="text-xs font-bold uppercase tracking-wider text-slate-500 dark:text-[#A1A1AA] flex items-center gap-1.5">
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="rounded-xl border border-rose-200 dark:border-rose-900/50 bg-rose-50 dark:bg-rose-950/30 px-3.5 py-1.5 text-xs font-bold text-rose-800 dark:text-rose-300 transition hover:bg-rose-100 dark:hover:bg-rose-900/40"
            >
              ☀️ {t("sim.severe_heatwave", "Severe Heatwave (+36°C)")}
            </button>
            <button
              onClick={applyCoolingPreset}
              className="rounded-xl border border-sky-200 dark:border-sky-900/50 bg-sky-50 dark:bg-sky-950/30 px-3.5 py-1.5 text-xs font-bold text-sky-800 dark:text-sky-300 transition hover:bg-sky-100 dark:hover:bg-sky-900/40"
            >
              ❄️ {t("sim.active_cooling", "Active Cooling (-6 THI)")}
            </button>
            <button
              onClick={applyOptimizedFeedPreset}
              className="rounded-xl border border-emerald-200 dark:border-emerald-900/50 bg-emerald-50 dark:bg-emerald-500/10 px-3.5 py-1.5 text-xs font-bold text-emerald-800 dark:text-emerald-300 transition hover:bg-emerald-100 dark:hover:bg-emerald-900/40"
            >
              🌾 {t("sim.high_energy_ration", "High-Energy Ration (+4kg)")}
            </button>
            <button
              onClick={resetBaseline}
              className="rounded-xl border border-slate-200 dark:border-[#27272A] bg-slate-100 dark:bg-[#1B1D20] px-3.5 py-1.5 text-xs font-bold text-slate-700 dark:text-[#F4F4F5] transition hover:bg-slate-200 dark:hover:bg-[#151719]"
            >
        ...
        <div className="grid gap-6 lg:grid-cols-12">
          ...
          <div className="lg:col-span-5 space-y-6 rounded-3xl border border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#151719] p-6 shadow-sm">
            ...
            {mode === "cow" && (
              <div className="space-y-2">
                <label className="text-xs font-bold text-slate-700 dark:text-[#F4F4F5] flex items-center gap-1.5">
                  <User className="h-4 w-4 text-emerald-600 dark:text-emerald-400" />
                ...
                <select
                  ...
                  className="w-full rounded-2xl border border-slate-200 dark:border-[#27272A] bg-slate-50 dark:bg-[#1B1D20] px-4 py-2.5 text-sm font-bold text-slate-900 dark:text-[#F4F4F5] focus:border-emerald-500 focus:bg-white dark:focus:bg-[#1B1D20] focus:outline-none"
                >
                  {cows.length === 0 ? (
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-black text-slate-900 dark:text-[#F4F4F5] bg-slate-100 dark:bg-[#1B1D20] px-2 py-0.5 rounded-lg border border-transparent dark:border-[#27272A]">
                  {temperature}°C
                </span>
              </div>
              <input
                ...
                className="w-full h-2 bg-slate-200 dark:bg-[#1B1D20] rounded-lg appearance-none cursor-pointer accent-rose-600"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full h-2 bg-slate-200 dark:bg-[#1B1D20] rounded-lg appearance-none cursor-pointer accent-sky-600"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full h-2 bg-slate-200 dark:bg-[#1B1D20] rounded-lg appearance-none cursor-pointer accent-amber-600"
              />
            </div>
            ...
            <div className="space-y-2">
              ...
              <input
                ...
                className="w-full h-2 bg-slate-200 dark:bg-[#1B1D20] rounded-lg appearance-none cursor-pointer accent-emerald-600"
              />
            </div>

            {/* Optional Scenario Economics Toggle & Controls */}
            <div className="border-t border-slate-100 dark:border-[#27272A] pt-4 space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-xs font-bold text-slate-800 dark:text-[#F4F4F5] flex items-center gap-1.5 cursor-pointer">
                ...
                <input
                  ...
                  className="rounded border-slate-300 dark:border-[#27272A] dark:bg-[#1B1D20] text-emerald-600 focus:ring-emerald-500 h-4 w-4"
                />
              </div>
              ...
              {useCustomPrices && (
                <div className="grid grid-cols-2 gap-3 pt-1 animate-in fade-in">
                  <div>
                    <label className="text-[11px] font-bold text-slate-600 dark:text-[#A1A1AA] block mb-1">
                      Milk Price (₹/L)
                    </label>
                    <input
                      type="number"
                      ...
                      className="w-full rounded-xl border border-slate-200 dark:border-[#27272A] bg-slate-50 dark:bg-[#1B1D20] px-3 py-1.5 text-xs font-bold text-slate-900 dark:text-[#F4F4F5] focus:bg-white dark:focus:bg-[#151719]"
                    />
                  </div>
                  <div>
                    <label className="text-[11px] font-bold text-slate-600 dark:text-[#A1A1AA] block mb-1">
            ...
            <button
              ...
              disabled={isLoading}
              className="w-full rounded-2xl bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 text-sm transition shadow-sm flex items-center justify-center gap-2"
            >
              {isLoading ? (
                <>
          ...
          <div className="lg:col-span-7 space-y-6">
            {isLoading ? (
              <div className="flex flex-col items-center justify-center rounded-3xl border border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#151719] p-12 text-center h-full min-h-[400px]">
                <RefreshCw className="h-8 w-8 animate-spin text-emerald-600 mb-3" />
                <h4 className="text-base font-bold text-slate-800 dark:text-[#F4F4F5]">{t("sim.calculating", "Calculating Digital Twin Scenario…")}</h4>
                <p className="text-xs text-slate-500 dark:text-[#A1A1AA]">{t("sim.calculating_desc", "Evaluating milk yield delta, thermal stress, and financial impact.")}</p>
              </div>
            ) : mode === "cow" && cowResult ? (
              <>
                ...
                <div className="rounded-3xl border border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#151719] p-6 shadow-sm space-y-6">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-[#27272A] pb-4">
                    <div>
                      <h3 className="text-lg font-black text-slate-900 dark:text-[#F4F4F5] flex items-center gap-2">
                        🐄 {cowResult.cow_name}
                      </h3>
                      <p className="text-xs text-slate-500 dark:text-[#A1A1AA] mt-0.5">
                    ...
                    <div
                      className={`rounded-2xl border p-4 transition ${
                        cowResult.delta_milk_yield_l >= 0
                          ? "border-emerald-200 dark:border-emerald-900/50 bg-gradient-to-br from-emerald-50 to-white dark:from-[#1B1D20] dark:to-[#151719]"
                          : "border-rose-200 dark:border-rose-900/50 bg-gradient-to-br from-rose-50 to-white dark:from-[#1B1D20] dark:to-[#151719]"
                      }`}
                    >
                      ...
                      <div className="mt-2 flex items-baseline gap-2">
                        ...
                        <span
                          className={`text-xs font-black flex items-center ${
                            cowResult.delta_milk_yield_l >= 0 ? "text-emerald-700 dark:text-emerald-400" : "text-rose-700 dark:text-rose-400"
                          }`}
                        >
                          {cowResult.delta_milk_yield_l >= 0 ? (
                  ...
                  <div className="rounded-2xl border border-slate-100 dark:border-[#27272A] bg-slate-50 dark:bg-[#1B1D20] p-4 space-y-3">
                    ...
                    <div className="grid grid-cols-2 gap-4 text-xs">
                      <div>
                        <span className="text-slate-500 dark:text-[#A1A1AA] block">Baseline THI</span>
                  ...
                  <div className="rounded-3xl border border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#151719] p-5 shadow-sm space-y-3">
                    ...
                    <div className="space-y-2">
                      {cowResult.recommendations.map((rec, idx) => (
                        <div
                          key={idx}
                          className="rounded-2xl border border-slate-100 dark:border-[#27272A] bg-slate-50 dark:bg-[#1B1D20] p-3.5 text-xs font-medium text-slate-800 dark:text-[#F4F4F5]"
                        >
                          <div className="font-bold text-slate-900 dark:text-[#F4F4F5]">{rec.title}</div>
                          <div className="mt-1 text-slate-600 dark:text-[#A1A1AA]">{rec.description}</div>
                        </div>
                      ))}
                    </div>
                  </div>
                    ...
                    <div className="mt-3 flex items-baseline gap-2">
                      ...
                      <span
                        className={`text-sm font-bold flex items-center ${
                          (herdResult.total_delta_l || 0) >= 0 ? "text-emerald-700 dark:text-emerald-400" : "text-rose-700 dark:text-rose-400"
                        }`}
                      >
                        {(herdResult.total_delta_l || 0) >= 0 ? (
                    ...
                    <table className="w-full text-left text-xs">
                      ...
                      <tbody className="divide-y divide-slate-100 dark:divide-[#27272A]">
                        {herdResult.cow_comparisons.map((c) => (
                          <tr key={c.cow_id} className="hover:bg-slate-50/80 dark:hover:bg-[#1B1D20]/60 transition">
                            <td className="py-3 font-bold text-slate-900 dark:text-[#F4F4F5]">
                              {c.cow_name} <span className="text-slate-400 dark:text-[#A1A1AA] font-normal">({c.tag_id})</span>
                            </td>
                            <td className="py-3 text-slate-600 dark:text-[#A1A1AA] font-medium">{c.baseline_yield_l.toFixed(1)} L</td>
                            <td className="py-3 font-bold text-slate-900 dark:text-[#F4F4F5]">{c.simulated_yield_l.toFixed(1)} L</td>
                            <td className={`py-3 font-bold ${c.delta_yield_l >= 0 ? "text-emerald-600 dark:text-emerald-400" : "text-rose-600 dark:text-rose-400"}`}>
                              {c.delta_yield_l >= 0 ? "+" : ""}{c.delta_yield_l.toFixed(1)} L ({c.percent_change.toFixed(1)}%)
                            </td>
                            <td className="py-3 text-slate-600 dark:text-[#A1A1AA] font-medium">
                              {c.baseline_thi} → <span className="font-bold text-slate-900 dark:text-[#F4F4F5]">{c.simulated_thi}</span>
                            </td>
                            <td className="py-3">
                              <span
