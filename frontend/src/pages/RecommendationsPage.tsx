      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h2 className="text-2xl font-semibold text-slate-900 dark:text-[#F4F4F5]">
              {t("recommendations.title", "AI Herd Recommendations")}
            </h2>
            <p className="text-sm text-slate-500 dark:text-[#A1A1AA]">
              {t("recommendations.subtitle", "Actionable, data-driven suggestions for nutrition, climate control, and lactation management.")}
            </p>
          </div>
          <button
            type="button"
          ...
          >
            {generateMutation.isPending ? t("recommendations.evaluating", "Evaluating Herd...") : `💡 ${t("recommendations.evaluate", "Evaluate Herd Recommendations")}`}
          </button>
        </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
          <div className="rounded-2xl border border-sky-100 dark:border-sky-900/40 bg-sky-50/40 dark:bg-sky-950/20 p-5 shadow-sm">
            <div className="text-xs font-medium text-sky-800 dark:text-sky-300">{t("recommendations.pending_actions", "Pending Actions")}</div>
            <div className="mt-2 text-3xl font-bold text-sky-950 dark:text-sky-200">
              {counts.pending}
            </div>
            <p className="mt-1 text-xs text-sky-700 dark:text-sky-400">{t("recommendations.pending_actions_desc", "Advisory items needing attention")}</p>
          </div>

          <div className="rounded-2xl border border-rose-100 dark:border-rose-900/40 bg-rose-50/40 dark:bg-rose-950/20 p-5 shadow-sm">
            <div className="text-xs font-medium text-rose-800 dark:text-rose-300">{t("recommendations.high_priority_actions", "High Priority")}</div>
            <div className="mt-2 text-3xl font-bold text-rose-950 dark:text-rose-200">
              {counts.high}
            </div>
            <p className="mt-1 text-xs text-rose-700 dark:text-rose-400">{t("recommendations.high_priority_desc", "Urgent health & feed actions")}</p>
          </div>
          ...
          <div className="rounded-2xl border border-emerald-100 dark:border-emerald-900/40 bg-emerald-50/40 dark:bg-emerald-500/[0.08] p-5 shadow-sm">
            <div className="text-xs font-medium text-emerald-800 dark:text-emerald-300">{t("recommendations.completed_actions", "Completed Actions")}</div>
            <div className="mt-2 text-3xl font-bold text-emerald-950 dark:text-emerald-200">
              {counts.completed}
            </div>
            <p className="mt-1 text-xs text-emerald-700 dark:text-emerald-400">{t("recommendations.completed_actions_desc", "Actioned herd advisories")}</p>
          </div>

          <div className="rounded-2xl border border-slate-200 dark:border-[#27272A] bg-white dark:bg-[#151719] p-5 shadow-sm">
            <div className="text-xs font-medium text-slate-500 dark:text-[#A1A1AA]">{t("recommendations.total_advisories", "Total Advisories")}</div>
            <div className="mt-2 text-3xl font-bold text-slate-900 dark:text-[#F4F4F5]">
              {counts.total}
            </div>
            <p className="mt-1 text-xs text-slate-400 dark:text-[#A1A1AA]/70">{t("recommendations.total_advisories_desc", "Total recommendations tracked")}</p>
          </div>
        </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          ...
          >
            {categories.map((option) => (
              <option key={option} value={option}>
                {option === "All" ? t("recommendations.all_categories", "All Categories") : option}
              </option>
            ))}
          </select>
          <select
          ...
          >
            {priorities.map((option) => (
              <option key={option} value={option}>
                {option === "All" ? t("recommendations.all_priorities", "All Priorities") : option}
              </option>
            ))}
          </select>
          <select
          ...
          >
            {completionStates.map((option) => (
              <option key={option} value={option}>
                {option === "all"
                  ? t("recommendations.all_statuses", "All Statuses")
                  : option === "pending"
                    ? t("recommendations.pending_only", "Pending Only")
                    : t("recommendations.completed_only", "Completed Only")}
              </option>
            ))}
          </select>
          <input
            ...
            placeholder={t("recommendations.search_placeholder", "Search action title or category...")}
